...
# ╔═╡ c2e7d9d0-3916-11ed-3d44-f3f608ed9c73
begin
  using DifferentialEquations, Plots, Distributions, Random
  theme(:dracula)
end

# ╔═╡ 168bdc23-bc52-486e-b07d-8e67ebac3933
md"""
# Dinámica de gases.
"""

# ╔═╡ e7d93e16-2c36-4f60-8e58-ea640b7dbebb
md"""Vamos a estudiar un sistema de muchas partículas moviéndose en una caja. Para simplificar, trabajaremos con una caja en el plano.

#### Fuerzas

Dadas dos masas $m_i$ y $m_j$ ubicadas en los puntos de coordenadas $(x_i,y_i)$ y $(x_j,y_j)$ respectivamente, podemos modelar los choques entre ellas a través de un potencial repulsivo. Proponemos un potencial de la forma:

$E^P_{ij} = k_1\frac{m_i m_j}{d_{ij}^4},$

donde $k_1$ es una constante de la interación y $d_{ij}$ es la distancia entre las partículas. 
...
# ╔═╡ 8d48d9b8-7989-4615-849a-c54fbb13f6ed
md"""### Primera Parte:

Realizamos la simulación general de este fenómeno. Asumimos que la caja es cuadrada y para reducir el número de variables definimos el lado como una variable global:"""

# ╔═╡ e3b0f74d-5f27-423e-a04f-06ac5f68a6e2
begin
	const L = 10.0
end

# ╔═╡ 001a6b5f-01d3-4634-94dd-25caca6026e0
md"""
Implementamos una función que genera un dato inicial aleatorio. 
Recibe un parámetro $n$ (la cantidad de partículas a generar) y un parámetro de escala `V`. Esta función genera, de manera aleatoria, las posiciones iniciales de cada una de las particulas.
"""

# ╔═╡ 87808e0f-3056-4b91-874b-788c1db84ee2
# dato inicial
function datoinicial(n, V)
  x = rand(Uniform(0, L), n)
  y = rand(Uniform(0, L), n)
  α = rand(Uniform(0,2pi), n) # Ángulo al azar entre 0 y 2pi
  v = rand(Uniform(0,V), n) # valor al azar entre 0 y V
  vx = v.*cos.(α)
  vy = v.*sin.(α)
  return transpose(reduce(hcat, [x, y, vx, vy]))
end

# ╔═╡ 0e507a2c-55f0-4573-bb94-c548ca18d34f
md"""
Implementamos la función que define el sistema de ecuaciones.
"""

# ╔═╡ 744b9c9c-d281-43f3-bd70-8fc1df7d0de4
 function sistema(du, u, p, t)
  ...
  for i in 1:n 
    for j in (i+1):n
        d_ij2 = (u[1,i] - u[1,j])^2 + (u[2,i] - u[2,j])^2  
        Fx[i,j] = 4*k₁*m[i]*m[j]* (u[1,i]-u[1,j]) / d_ij2^3
        Fx[j,i] = -Fx[i,j]
        Fy[i,j] = 4*k₁*m[i]*m[j]* (u[2,i]-u[2,j]) / d_ij2^3
        Fy[j,i] = -Fy[i,j]
    end
  end
  for i in 1:n
...
# ╔═╡ 0324b2a8-f81c-49ec-b0c9-8779991ab562
md"""
Implementamos una función para reconocer los choques contra las paredes.
"""

# ╔═╡ 462837f1-1472-44e2-ab2c-ec7d582c8a3a
function condicion_sefuederango(u, t, integrator)
	res = false
    for i in 1:integrator.p[1]
       res = res || (u[1,i] <= 0.0) || (u[1,i] >= L) || (u[2,i] <= 0.0) || (u[2,i] >= L)
    end
	return res
end

# ╔═╡ 0d66ac6e-3bdf-4029-98f6-1375e78ab2ca
function rebote!(integrator)
	for i in 1:integrator.p[1]
      if(integrator.u[1,i] <= 0)  
		  integrator.u[1,i] = -integrator.u[1,i]
		  integrator.u[3,i] = -integrator.u[3,i]
	  end
      if(integrator.u[1,i] >= L)  
		  integrator.u[1,i] = L - (integrator.u[1,i] - L) 
		  integrator.u[3,i] = -integrator.u[3,i]
	  end
      if(integrator.u[2,i] <= 0)  
		  integrator.u[2,i] = -integrator.u[2,i] 
	  	  integrator.u[4,i] = -integrator.u[4,i]
	  end
      if(integrator.u[2,i] >= L)  
		  integrator.u[2,i] = L - (integrator.u[2,i] - L)
	  	  integrator.u[4,i] = -integrator.u[4,i]
	  end
	end
end

# ╔═╡ a273fcc0-cf18-4740-9ee3-da24b8a1a9c3
md"""
Implementamos una función que ejecute una simulación.
"""

# ╔═╡ 667fe413-a755-450d-a5f8-0fbdeffd27c8
# simulación
function simulacion(n, k1, masasiguales, masa, V)
	t = [0.0, 20.0]
	if (masasiguales == 1)
		masas₀ = ones(n)*masa
	else
	    masas₀ = rand(Uniform(0.5, 2.5), n)
	end
	p₀ = [n, k1, masas₀]
	u₀ = datoinicial(p₀[1], V)
	P₀ = ODEProblem(sistema, u₀, t, p₀)
	dc = DiscreteCallback(condicion_sefuederango, rebote!)
	sol₀ = solve(P₀, callback = dc, dtmax = 0.01)
	return [sol₀, masas₀]	
end

# ╔═╡ 266a54ef-98a2-44e1-ac0b-f22c917974c0
md"""
Visualizamos el resultado de una simulación tomando $k_1 = 10^{-6}$ y probamos con diferentes valores iniciales.
"""

# ╔═╡ ce2979b8-a04a-4cb6-b3b1-2d5d748059ba
function visualizacion(sol₀,tams, n)
	#Animacion del movimiento
	@gif for i in 1:0.1:20
	   r = sol₀(i)
		scatter(r[1, :], r[2, :], xlim = (0, L), ylim = (0, L), c= 1:n,label = "", markersize  = tams*3.5)
	end
end

# ╔═╡ 83e82a05-50ed-429f-80eb-e8c691fa3c0f
# prueba con 2
begin
	sol_2, masas_2 = simulacion(2, 10^-6, 1, 1.0, 2)
	visualizacion(sol_2,masas_2,2)
end

# ╔═╡ f334dd64-463e-4c4f-b146-e1f0681e566c
# prueba con 5
begin
	sol_5, masas_5 = simulacion(5, 10^-6, 1, 1.0, 2)
	visualizacion(sol_5,masas_5, 5)
end

# ╔═╡ cbaae9fb-f06d-440c-828e-38aa1b12ceb9
# prueba con 10
begin
	sol_10, masas_10 = simulacion(10, 10^-6, 1, 1.0, 2)
	visualizacion(sol_10,masas_10, 10)
end

# ╔═╡ e175f6d1-069e-41b7-a756-ab71991c8b8f
# prueba con 30 masas distintas
begin
	sol_30_distintas, masas_30_distintas = simulacion(30, 10^-6, 0, 0, 2)
	visualizacion(sol_30_distintas, masas_30_distintas,30)
end

# ╔═╡ 7b4720b3-1e9c-4a02-b3ef-9f50966252ba
md"""##### Segunda Parte:

"""

# ╔═╡ e5ed6302-0c38-4266-8705-805d9fd9ffbb
md"""###### Presión:

La presión a la que se encuentra el gas es proporcional a la cantidad de rebotes de partículas contra las paredes de la caja. Modificamos las funciones para incorporar la presión.
"""

# ╔═╡ 204cb447-913f-445a-aa5c-f8be1fa662f1
function presion(du, u, p, t)
  n, k₁, m, contador = p
  Fx = zeros(n,n) 
  Fy = zeros(n,n)
  for i in 1:n 
...
# ╔═╡ c66b1b76-ff41-48d4-ab05-643f715610be
function reboteconContador!(integrator)
	for i in 1:integrator.p[1]
      if(integrator.u[1,i] <= 0)  
		  integrator.u[1,i] = -integrator.u[1,i]
		  integrator.u[3,i] = -integrator.u[3,i]
...
# ╔═╡ d76002f3-8e9a-4f19-961b-edd051b37161
# dato inicial
function datoinicialModificado(n, V)
  x = rand(Uniform(0, L), n)
  y = rand(Uniform(0, L), n)
  α = rand(Uniform(0,2pi), n) # Ángulo al azar entre 0 y 2pi
  vx = V.*cos.(α)
...
# ╔═╡ 5e3f3416-57e8-40f2-a6d9-44c868202fd9
function presionMismaVelocidad(n, V)
	t = [0.0, 20.0]
	u₀ = datoinicialModificado(n, V)
	masas = ones(n)
	p = [n, 0.002, masas, 0]
	P = ODEProblem(presion, u₀, t, p)
	dc = DiscreteCallback(condicion_sefuederango, reboteconContador!)
	sol = solve(P, callback = dc, dtmax = 0.01)
	return p[4]
end

# ╔═╡ 609adf84-2b67-4d6c-96e6-77decdd94048
md"""
Buscamos visualizar la cantidad de rebotes en función de la cantidad de partículas y en función de la velocidad. Para esto, corremos, para cada valor, cuatro veces y promediamos la cantidad de rebotes.
Debido a la aleatoriedad de los datos iniciales, los gráficos pueden presentar picos para ciertos valores pero se ve un aumento en la cantidad de rebotes a medida que aumenta la velocidad o las partículas."""

# ╔═╡ 10304c4e-d7aa-48ed-b1e7-0bd117486124
begin
	V = 3
	rebotes = []
	rangoN = 1:20
	for i in rangoN
		cantidad1 = presionMismaVelocidad(i, V)
		cantidad2 = presionMismaVelocidad(i, V)
		cantidad3 = presionMismaVelocidad(i, V)
		cantidad4 = presionMismaVelocidad(i, V)
		cantidadPromedio = (cantidad1 + cantidad2 + cantidad3 + cantidad4)/4
		push!(rebotes, cantidadPromedio)
	end	 
	plot(rangoN, rebotes, xlabel="Particulas", ylabel="Rebotes" , label ="", title = "Cantidad de rebotes según la cantidad de particulas")
end

# ╔═╡ 7334fb97-1f71-4ed1-9f3e-6cf923d2d503
begin
	n = 10
	rebotes1 = []
	rangoV = 1:0.5:10
	for i in rangoV
		cantidad1 = presionMismaVelocidad(n, i)
		cantidad2 = presionMismaVelocidad(n, i)
		cantidad3 = presionMismaVelocidad(n, i)
		cantidad4 = presionMismaVelocidad(n, i)
		cantidadPromedio = (cantidad1 + cantidad2 + cantidad3 + cantidad4)/4
		push!(rebotes1, cantidadPromedio)
	end	 
	plot(rangoV, rebotes1, xlabel="Velocidad", ylabel="Rebotes", label = "",
    title = "Cantidad de rebotes según la velocidad")
end
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "658339e54985de5ec085840efd31c040b71ddf02"
